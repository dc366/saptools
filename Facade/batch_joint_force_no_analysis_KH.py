...
for folder in folder_list:
    ...
    """
        ...
        #output joint forces on mem_bj-start.in and mem_bj-in
    try:
        member_results = a.SapModel.Results.FrameJointForce("mem_king hat", 2)
    except:
        print("  member force extract error MEM_king hat "+file_name)
    else:
        ret = member_results[-1]
        ret = checkret(ret,"output mem_king hatn "+file_name)
    ...
    #output joint connectivity for mem_king hat
    a.select_group("mem_king hat")
    bj_members,_ = a.get_list_sap('frame')
